...
design="dsp_multiplier_accum_with_add_neg"
...
#creating a tcl file to run raptor flow 
    ...
    if [ "$post_synth_sim" == true ]; then 
        ...
        [ "$tool_name" = "iverilog" ] && echo "simulation_options compilation icarus gate" >> raptor_tcl.tcl || echo "simulation_options compilation verilator --main gate" >> raptor_tcl.tcl
