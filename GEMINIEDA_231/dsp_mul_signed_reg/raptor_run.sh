...
function simulate(){
    ...
    dsp_path=`find $library -wholename "*/rapidsilicon/genesis/dsp_sim.v"`
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        vcs -sverilog $cell_path $dsp_path $dsp_final_map $dsp_map $lut_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $design_path ../$design/$design\_post\_synth.v $tb_path +incdir+$directory_path -y $directory_path +libext+.v -full64 -debug_all 2>&1 | tee post_synth_sim.log
