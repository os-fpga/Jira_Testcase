...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/simpleuart.v">>raptor_tcl.tcl || echo ""
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/progmem.v">>raptor_tcl.tcl || echo ""
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/picosoc_noflash.v">>raptor_tcl.tcl || echo ""
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/picorv32.v">>raptor_tcl.tcl || echo ""
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/picosoc_symbiflow.v">>raptor_tcl.tcl || echo ""
    ##vary design to design

    echo "set_top_module $design">>raptor_tcl.tcl 

    ##vary design to design
    echo "pin_loc_assign_method free">>raptor_tcl.tcl 
    ...
    echo "set_limits bram 0">>raptor_tcl.tcl  
...
synth_netlist_path=`find . -type f -iname "$design\_post_synth.v" -printf $main_path/'%p\n'`
...
#renaming netlist module name in post synth netlist
    ...
    then
        ...
        while read line; do
            # for word in $line; do
                ...
                then
                    sed -i "s/module $design/module $design\_post_synth/" $synth_netlist_path
                    break 2
                fi
    ...
    then
        ...
        while read line; do
            # for word in $line; do
                if [[ $(echo "$line" | cut -d "(" -f1)  == "module $design " ]]; #grep -F "module $design" $synth_netlist_path
                then
...
if [[ $compile_opts == "post_route_sim" ]]
    ...
    while read line; do
            # for word in $line; do
                if [[ $(echo "$line" | cut -d "(" -f1)  == *"_post_synth netlist" ]]; #grep -F "module $design" $synth_netlist_path
                then
...
if [[ $compile_opts == "post_synth_sim" ]]
    ...
    while read line; do
            # for word in $line; do
                if [[ $(echo "$line" | cut -d "(" -f1)  == *"_post_route netlist" ]]; #grep -F "module $design" $synth_netlist_path
                then
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        [ ! -d $design\_$tool_name\_post_synth_files ] && mkdir $design\_$tool_name\_post_synth_files
        [ -d $design\_$tool_name\_post_synth_files ] && cd $design\_$tool_name\_post_synth_files
        (cd ../../rtl && timeout 10m iverilog -g2012 -o $design $primitive_sim $design_path $synth_netlist_path $tb_path -y $directory_path && timeout 5m vvp ./$design && mv $design tb.vcd -t ../results_dir/$design\_$tool_name\_post_synth_files) 2>&1 | tee post_synth_sim.log
