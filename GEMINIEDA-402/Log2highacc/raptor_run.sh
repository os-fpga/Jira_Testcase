...
# while [ "$x" = "true" ]; do
# main_path=$(cd .. && pwd)
# if [[ $main_path == *"Compiler_Validation" ]] 
# then
# x="false"
# fi
# cd ..
...
#creating a tcl file to run raptor flow 
    ...
    echo "add_include_path ./rtl">>raptor_tcl.tcl
    echo "add_library_path ./rtl">>raptor_tcl.tcl
    echo "add_design_file ./rtl/$design.v">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        vcs -sverilog $cell_path $lut_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $design_path ../$design/$design\_post\_synth.v $tb_path +incdir+$directory_path -y $directory_path +libext+.v -full64 -debug_all -lca -kdb 2>&1 | tee post_synth_sim.log
    ...
    then
        ...
        echo "    sc_start(200, SC_MS);">>tb_$design.cpp
        echo "    return 0;">>tb_$design.cpp
        echo "}">>tb_$design.cpp
        verilator -Wno-fatal -Wno-BLKANDNBLK -sc -exe $tb_path tb_$design.cpp --timing --timescale 1ns/1ps --trace -v $cell_path -v $lut_map -v $TDP18K_FIFO -v $ufifo_ctl -v $dsp_sim -v $sram1024x18 -v $design_path -v ../$design/$design\_post\_synth.v -y $directory_path +libext+.v+.sv 2>&1 | tee post_synth_sim.log
        make -j -C obj_dir -f Vco_sim_$design.mk Vco_sim_$design 2>&1 | tee -a post_synth_sim.log
        obj_dir/Vco_sim_$design 2>&1 | tee -a post_synth_sim.log
        cd ..
    fi
}
