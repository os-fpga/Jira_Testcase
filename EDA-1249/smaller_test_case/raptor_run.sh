...
post_route_sim=true #design_level
...
parse_cga exit 1; }
    ...
    primitives=`find $library -wholename "*/rapidsilicon/genesis3/primitives.v"`
    compile_opts=$1    
...
#finding the co simulation testbench for the design given
    tb_path=`find $PWD -type f -iname "co_sim_$design.sv"`
    if [ -z "$tb_path" ]
    then
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        (cd ../../rtl && timeout 4m vcs -sverilog $primitives $cell_path $bram_sim $lut_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $dsp_sim $design_path ../results_dir/$design/$design\_post\_synthesis.v $tb_path +incdir+$directory_path -y $directory_path +libext+.v +define+VCS_MODE=1 -full64 -debug_all -lca -kdb && ./simv && mv simv *.vcd *.key *.log verdi_config_file csrc simv.daidir -t ../results_dir/$design\_$tool_name\_post_route_files) 2>&1 | tee post_route_sim.log
		while read line; do
                if [[ $line == *"All Comparison Matched"* ]]
                then
                    rm -fr tb.vcd
