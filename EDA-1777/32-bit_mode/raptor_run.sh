...
function end_time(){
    ...
    function parse_cga(){
    	...
    	timeout 5m python3 ../../../parser.py ./results_dir/results.log ./results_dir/raptor_perf.log
...
parse_cga exit 1; }
    ...
    lut_map=`find $library -wholename "*/rapidsilicon/genesis3/simlib.v"`
    ...
    post_synth_file=$(find $main_path -type f -iname "$design\_post\_synth.v")
    post_route_file=$(find $main_path -type f -iname "$design\_post\_synthesis.v")
...
#renaming netlist module name in post synth netlist
    ...
    then
        ...
        while read line; do
            # for word in $line; do
                ...
                then
                    sed -i "s/module $design/module $design\_post_synth/" $post_synth_file
                    break 2
                fi
    ...
    then
        ...
        while read line; do
            # for word in $line; do
                if [[ $(echo "$line" | cut -d "(" -f1)  == "module $design " ]]; #grep -F "module $design" $post_synth_file
                then
                    sed -i "s/module $design/module $design\_post_route/" $post_route_file
                    break 2
                fi
...
if [[ $compile_opts == "post_route_sim" ]]
    ...
    while read line; do
            # for word in $line; do
                if [[ $(echo "$line" | cut -d "(" -f1)  == *"_post_synth netlist" ]]; #grep -F "module $design" $post_synth_file
                then
...
if [[ $compile_opts == "post_synth_sim" ]]
    ...
    while read line; do
            # for word in $line; do
                if [[ $(echo "$line" | cut -d "(" -f1)  == *"_post_route netlist" ]]; #grep -F "module $design" $post_synth_file
                then
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        (cd ../../rtl && timeout 4m vcs -sverilog $cell_path $bram_sim $lut_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $dsp_sim $design_path $post_synth_file $tb_path +incdir+$directory_path -y $directory_path +libext+.v -full64 -debug_all -lca -kdb && timeout 5m ./simv && mv simv *.vcd *.key *.log verdi_config_file csrc simv.daidir -t ../results_dir/$design\_$tool_name\_post_synth_files) 2>&1 | tee post_synth_sim.log
		while read line; do
    ...
    then
        [ ! -d $design\_$tool_name\_post_synth_files ] && mkdir $design\_$tool_name\_post_synth_files
        [ -d $design\_$tool_name\_post_synth_files ] && cd $design\_$tool_name\_post_synth_files
        (cd ../../rtl && iverilog -g2012 -o $design $cell_path $bram_sim $lut_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $dsp_sim $design_path $post_synth_file $tb_path +incdir+$directory_path -y $directory_path +libext+.v && vvp ./$design && mv $design tb.vcd -t ../results_dir/$design\_$tool_name\_post_synth_files) 2>&1 | tee post_synth_sim.log
		while read line; do
                if [[ $line == *"All Comparison Matched"* ]]
                then
                    rm -fr tb.vcd
