...
design="read_netlist_blif_accumulator"
...
function compile () {
    ...
    design_path=`find $temp -type f -iname "$design.blif"`
    if [ -z "$design_path" ]
    then
...
#creating a tcl file to run raptor flow 
    ...
    echo "create_design $design">raptor_tcl1.tcl 
    echo "target_device $device">>raptor_tcl1.tcl 
    ...
    [ -z "$ip_name" ] && echo "add_include_path ./rtl">>raptor_tcl1.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_library_path ./rtl">>raptor_tcl1.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_library_ext .v .sv">>raptor_tcl1.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/$design.v">>raptor_tcl1.tcl || echo "" 
    ##vary design to design

    echo "set_top_module $design">>raptor_tcl1.tcl 

    ##vary design to design
    [ -z "$add_constraint_file" ] && echo "" || echo "add_constraint_file $add_constraint_file">>raptor_tcl1.tcl #design_level
    ##vary design to design

    [ -z "$verific_parser" ] && echo "" || echo "verific_parser $verific_parser">>raptor_tcl1.tcl
    [ -z "$synthesis_type" ] && echo "" || echo "synthesis_type $synthesis_type">>raptor_tcl1.tcl
    [ -z "$custom_synth_script" ] && echo "" || echo "custom_synth_script $custom_synth_script">>raptor_tcl1.tcl
    [ -z "$synth_options" ] && echo "" || echo "synth_options $synth_options">>raptor_tcl1.tcl
    [ -z "$strategy" ] && echo "" || echo "synthesize $strategy">>raptor_tcl1.tcl 
    rm -fR blif_temp 
    mkdir blif_temp
    cd blif_temp
    raptor --batch --script ../raptor_tcl1.tcl 2>&1 | tee raptor.log

    mv $design/$design\_post\_synth.blif ../rtl

    cd ..

    echo "create_design $design -type gate-level">raptor_tcl.tcl 
    ...
    [ -z "$ip_name" ] && echo "read_netlist ./rtl/$design\_post\_synth.blif">>raptor_tcl.tcl || echo "" 
    ##vary design to design
