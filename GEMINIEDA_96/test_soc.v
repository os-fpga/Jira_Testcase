...
assign builder_shared_err = ((main_basesoc_ram_bus_err | main_ram_bus_ram_bus_err) | builder_basesoc_wishbone_err);
assign builder_wait = ((builder_shared_stb & builder_shared_cyc) & (~builder_shared_ack));
always @(*) begin
	builder_shared_dat_r <= 32'd0;
	builder_error <= 1'd0;
	builder_shared_ack <= 1'd0;
	builder_shared_ack <= ((main_basesoc_ram_bus_ack | main_ram_bus_ram_bus_ack) | builder_basesoc_wishbone_ack);
	builder_shared_dat_r <= ((({32{builder_slave_sel_r[0]}} & main_basesoc_ram_bus_dat_r) | ({32{builder_slave_sel_r[1]}} & main_ram_bus_ram_bus_dat_r)) | ({32{builder_slave_sel_r[2]}} & builder_basesoc_wishbone_dat_r));
...
always @(posedge sys_clk) begin
	...
	{main_tx_tick, main_tx_phase} <= 26'd49478023;
	if (main_tx_enable) begin
		{main_tx_tick, main_tx_phase} <= (main_tx_phase + 26'd49478023);
	end
	...
	if (main_rx_enable) begin
		{main_rx_tick, main_rx_phase} <= (main_rx_phase + 26'd49478023);
	end
...
//------------------------------------------------------------------------------
// Memory rom: 5220-words x 32-bit
//------------------------------------------------------------------------------
// Port 0 | Read: Sync  | Write: ---- | 
reg [31:0] rom[0:5219];
initial begin
	$readmemh("test_soc_rom.init", rom);
end
reg [31:0] rom_dat0;
always @(posedge sys_clk) begin
...
initial begin
	$readmemh("test_soc_sram.init", sram);
end
reg [10:0] sram_adr0;
always @(posedge sys_clk) begin
	if (main_ram_we[0])
...
reg [7:0] mem[0:45];
initial begin
	$readmemh("test_soc_mem.init", mem);
end
reg [5:0] mem_adr0;
always @(posedge sys_clk) begin
...
// -----------------------------------------------------------------------------
//  Auto-Generated by LiteX on 2022-06-02 11:47:55.
//------------------------------------------------------------------------------
