...
design="RS_DSP_MULTACC_REGIN_const0_const1_registered_in_accum_out_comb"
tool_name="vcs"
...
#creating a tcl file to run raptor flow 
    ...
    echo "add_library_ext .v">>raptor_tcl.tcl
...
#running raptor flow script
    timeout $timeout raptor --batch --script $PWD/raptor_tcl.tcl 2>&1 | tee raptor.log
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        vcs -sverilog $cell_path $lut_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $design_path ../$design/$design\_post\_synth.v $tb_path +incdir+$directory_path -full64 -debug_all -kdb -lca 2>&1 | tee post_synth_sim.log
