...
design="RS_DSP_MULTADD_REGIN_REGOUT_const0_const1_add_shifted_a_to_mult_output_coeff3_in_and_out_registered"
tool_name="vcs"
...
#creating a tcl file to run raptor flow 
    ...
    echo "target_device GEMINI_TEST_DSP">>raptor_tcl.tcl
...
cd results_dir
echo "Device: GEMINI_TEST_DSP">>results.log
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        vcs -sverilog $cell_path $dsp_sim $lut_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $design_path ../$design/$design\_post\_synth.v $tb_path +incdir+$directory_path -y $directory_path +libext+.v -full64 -debug_all 2>&1 | tee post_synth_sim.log
